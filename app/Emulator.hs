...
showStatusFlags sreg =
  " I: "
    ++ show (interruptFlag sreg)
    ++ " T: "
    ++ show (tFlag sreg)
    ++ " H: "
    ++ show (halfCarryFlag sreg)
    ++ " S: "
    ++ show (signFlag sreg)
    ++ " V: "
    ++ show (overflowFlag sreg)
    ++ " N: "
    ++ show (negativeFlag sreg)
    ++ " Z: "
    ++ show (zeroFlag sreg)
    ++ " C: "
    ++ show (carryFlag sreg)
...
printRow addr values = do
  ...
  where
    ...
    printPair [a, b] = do
      ...
      putStr " "
    printPair [a] = do
      -- Handle the last unpaired byte if any
...
resolveLabels labelMap (address, JMP label)
  ...
  where
    relAddress = Map.findWithDefault 0 label labelMap - address
...
run instructions memorySize =
  let initialState =
   ...
   in runProgram instructionsWithAddresses initialState
